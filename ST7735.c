...
void ST7735_DrawBitmapReversed(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = w*(h - 1);
  ...
  int base = (h - 1 - row) * originalWidth;

  for(int col = 0; col < w; col++){
    int idx = base + (w - 1 - col);  // horizontal flip

    SPI_OutData((uint8_t)(image[idx] >> 8));
    SPI_OutData((uint8_t)image[idx]);
  }
}
